...
지금까지 useState => useReducer => useContext 이제 => redux

context: state, 내부함수(reducers)
redux: state, 내부함수(reducers), 외부함수(extraReducers: api)

RestApi: create, read, update, delete 다함(그냥 데이터를 넘기기만 하면 됨)
get(read) / get(url(주소("문자열"))) / get(http://localhost.0145/todo)
post(create) / post(주소, object) / post(http://localhost.0145/todo, object)
put(update) / put(주소, object)  put(http://localhost.0145/todo/id, object)
delete(delete) / delete(주소) / delete(http://localhost.0145/todo/id)


Restful API
get 방식: url => return: 테이블(json)
get 하나의 데이터: url+id => return: 오브젝트(행)
